...
export class CbToolbar extends LitElement {
  ...
  @state() private accessor _openMenu: 'player' | 'line' | 'equipment' | 'color' | 'cone-color' | 'line-color' | null = null;
    ...
    return html`
      ...
      <div class="dropdown-wrap">
        <button
          ...
          @click="${this.#toggleMenu.bind(this, 'player')}">
          <span class="icon">●</span> Add Player <span class="caret"></span>
        </button>
        ${this._openMenu === 'player' ? html`
          <div class="dropdown">
            ${TEAMS.map((t, i) => html`
              <button @click="${() => this.#pickPlayer(t.color, t.team)}">
                ${i === 0 ? html`
      ...
      <div class="dropdown-wrap">
        <button
          ...
          @click="${this.#toggleMenu.bind(this, 'equipment')}">
        ...
        ${this._openMenu === 'equipment' ? html`
          <div class="dropdown">
            <button @click="${() => this.#pickEquipment('ball')}">
            ...
            <button @click="${() => this.#pickEquipment('cone')}">
            ...
            <button @click="${() => this.#pickEquipment('coach')}">
              <svg viewBox="0 0 16 16" width="16" height="16" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0">
            ...
            <button @click="${() => this.#pickEquipment('goal')}">
              <svg viewBox="0 0 16 16" width="16" height="16" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0">
            ...
            <button @click="${() => this.#pickEquipment('mini-goal')}">
              <svg viewBox="0 0 16 16" width="16" height="16" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0">
      ...
      <div class="dropdown-wrap">
        <button
          ...
          @click="${this.#toggleMenu.bind(this, 'line')}">
        ...
        ${this._openMenu === 'line' ? html`
          <div class="dropdown">
            ${LINE_STYLES.map(s => html`
              <button @click="${() => this.#pickLine(s.value)}">
    ...
    return html`
      <div class="dropdown-wrap">
        <button class="color-btn"
                @click="${this.#toggleMenu.bind(this, 'color')}">
        ...
        ${this._openMenu === 'color' ? html`
          <div class="dropdown color-grid" style="right: 0; left: auto;">
            ${PLAYER_COLORS.map(c => html`
              <button aria-pressed="${refPlayer.color === c.color}"
  ...
  #renderConeEditor() {
    ...
    return html`
      ...
      <div class="player-editor">
        ...
        <div class="dropdown-wrap">
          <button class="color-btn"
                  @click="${this.#toggleMenu.bind(this, 'cone-color')}">
            <svg viewBox="0 0 16 16" width="16" height="16">
              <circle cx="8" cy="8" r="5" fill="#222" stroke="${refCone.color ?? '#7fff00'}" stroke-width="3" />
            </svg>
          </button>
          ${this._openMenu === 'cone-color' ? html`
            <div class="dropdown color-grid" style="right: 0; left: auto; grid-template-columns: repeat(2, 1fr);">
              ${CONE_COLORS.map(c => html`
                <button aria-pressed="${(refCone.color ?? '#7fff00') === c.color}"
      ...
      <div class="player-editor">
        ...
        <div class="dropdown-wrap">
          <button class="color-btn" title="Line color"
                  @click="${this.#toggleMenu.bind(this, 'line-color')}">
            <span class="color-swatch" style="background: ${ref.color}"></span>
          </button>
          ${this._openMenu === 'line-color' ? html`
            <div class="dropdown" style="right: 0; left: auto; flex-direction: row; gap: 4px; padding: 6px;">
              ${LINE_COLORS.map(c => html`
                <button style="width: 28px; height: 28px; padding: 0; display: flex; align-items: center; justify-content: center; border-radius: 6px;${ref.color === c.color ? ' background: #1a4a7a; border-color: white;' : ''}"
  ...
  #toggleMenu(menu: typeof this._openMenu, e: Event) {
    e.stopPropagation();
    this._openMenu = this._openMenu === menu ? null : menu;
  }
